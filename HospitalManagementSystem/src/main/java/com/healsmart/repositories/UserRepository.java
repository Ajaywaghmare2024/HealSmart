...
public interface UserRepository extends JpaRepository<User, Long> {
	...
	@Query(value = "update users u "
			+ "inner join employees e on e.user_id =u.userId "
			+ "set u.first_name = :firstName,u.last_name=:lastName ,"
			+ "e.dob=:dob,u.cell_no=:cellNo where u.id=:userId ",nativeQuery = true)
	Long updateFirstNameLastNameDobCellNo(@Param("firstName")String firstName,@Param("lastName") String lastName,
			@Param("dob") LocalDate dob,@Param("cellNo") String cellNo,
			@Param("userId") Long userId);
	
	
//delete employee using cell and userId
	@Modifying
	@Query(value = "delete from users u where u.userId=:userId and cell_no=:cellNo",nativeQuery = true)
	...
	@Modifying
	@Query(value = "insert into users values(:userId,:firstName,:lastName,:email"
			+ ",:password,:cellNo,:role,:securityQuestion,:securityAnswer)",nativeQuery = true)
	Long insertIntoUsers(@Param("userId") long userId,@Param("firstName") String firstName,
	...
	@Modifying
	@Query(value = "update users set first_name= :firstName,last_name= :lastName,cell_no= :cellNo where UserId= :userId",nativeQuery = true)
