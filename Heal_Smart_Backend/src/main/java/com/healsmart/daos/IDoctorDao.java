...
public interface IDoctorDao extends JpaRepository<Doctor, Integer> {
	...
	int insertIntoDoctorTable(@Param("id") int id,
			@Param("emp_id") int emp_id,
			@Param("charges")double charges);
	
	@Query(value = "select id from doctors where emp_id="
			+ "(select id from employees where user_id="
			+ "(select id from users where first_name=:firstName and last_name=:lastName))",nativeQuery = true)
	int getDoctorIdByFirstNameAndLastName(@Param("firstName") String firstName,
